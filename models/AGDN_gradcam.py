"""
In the AGDN paper, the attention maps used to guide image deformation are generated by averaging the
feature maps of the block 4 in the branch1.
In this script, we replace the average attention map with the grad-cam attention map, which is inspired by the paper:
...
class DenseNet:
    ...
    def save_path(self):
        try:
            ...
            save_path = 'saves/B2_gradcam/' + self.which_split
    ...
    def generate_gradcam(self, pred_labels, logits, f_maps):

        f_size = int(f_maps.get_shape()[-2])
        output_channels = int(f_maps.get_shape()[-1])

        # one_hot = tf.sparse_to_dense(pred_labels, [self.n_classes], 1.0)
        one_hot = tf.one_hot(pred_labels, self.n_classes)
        loss = tf.reduce_mean(tf.multiply(logits, one_hot))

        grads = tf.gradients(loss, f_maps)[0] # the same size as f_maps
        print("grad================", grads)
        # Normalizing the gradients\
        norm_grads = tf.div(grads, tf.sqrt(tf.reduce_mean(tf.square(grads))) + tf.constant(1e-5))

        weights = tf.reduce_mean(norm_grads, axis=(1, 2))  # (bs, 1, c)
        weights = tf.reshape(weights, [-1, 1, output_channels])

        f_maps = tf.reshape(f_maps, [-1, f_size * f_size, output_channels])
        cam = tf.matmul(f_maps, weights, adjoint_b=True)
        cam = tf.reshape(cam, [-1, f_size, f_size, 1])

        # normalize the cam map.
        cam = tf.nn.relu(cam)
        cam_min = tf.reduce_min(cam, axis = [1, 2, 3], keepdims = True) # [batch_size, 8, 8, 1]
        cam_max = tf.reduce_max(cam, axis = [1, 2, 3], keepdims = True)
        cam = tf.div(cam - cam_min + 1e-8, cam_max - cam_min + 1e-8) # [batch_size, 8, 8, 1]

        cam = tf.image.resize_images(cam, size = (31, 31)) # used for image resampling.
        cam_viz = tf.image.resize_images(tf.tile(cam, (1,1,1,3)), (128, 128)) # (8, 128, 128, 3), for visualization.
    ...
    def _build_graph(self):
        ...
        self.cam_att, self.viz_cam = self.generate_gradcam(self.pred_labels, logits1, f_maps1)
    ...
    def train_all_epochs(self, train_params):
        ...
        self.test_high_image_batch, self.test_low_image_batch, self.test_label_batch = \
                                    get_image_label_batch(batch_size, self.which_split, shuffle=False, name='test2')
        ...
        try:
            ...
            dataframe.to_csv("./acc_results/B2_gradcam/" + self.which_split + ".csv", index = True, sep = ',')
    ...
    def test(self, batch_size):
        ...
        input_path = os.path.join("./visualize_attention/gradcam_input2", self.which_split)
        # saliency_path = "./visualize_conf_maps/saliency_save/B2 + 3TOA/split2/"
        saliency_path = "./visualize_heatmaps/B2_gradcam"
        ...
        for i in range(self.num_test // batch_size):
            ...
            fetches = [self.input2, self.cross_entropy1, self.cross_entropy2, self.sum_loss, self.TE_loss, self.viz_cam, \
                        self.saliency1, self.saliency2, self.smaps1_b2, self.smaps1_b3, self.pred1, self.pred2, self.pred]

            input2, loss1, loss2, loss3, TE_loss, cam_att1, avg_att1, s_map2, smap1_b2, smap1_b3, pred1, pred2, pred = \
            ...
            for index in range(batch_size):
                ...
                if labels[index] != 0:
                    ...
                    save_img(cam_att1[index], img_index, saliency_path, img_name='_cam.jpg', split=self.which_split, mode = "heatmap")
                    save_img(avg_att1[index], img_index, saliency_path, img_name='_avg.jpg', split=self.which_split, mode = "heatmap")
            #
