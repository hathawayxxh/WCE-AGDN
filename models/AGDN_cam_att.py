...
In this script, we replace the average attention map with the class activation maps (CAM).
"""
...
class DenseNet:
    ...
    def save_path(self):
        try:
            ...
            save_path = 'saves/model_B2_cam/' + self.which_split
    ...
    def generate_cam(self, pred_labels, conv):
        """
        Generate class activation map for the target class.
        conv: the feature maps of the last block.
        pred_labels: the labels predicted by the first network.
        """
        print("generating class activation map.")
        f_size = int(conv.get_shape()[-2])
        output_channels = int(conv.get_shape()[-1])
        with tf.variable_scope('net1/Transition_to_classes', reuse=True):
            label_w = tf.gather(tf.transpose(tf.get_variable('weight')), pred_labels)
            label_w = tf.reshape(label_w, [-1, 1, output_channels])
        f_maps = tf.reshape(conv, [-1, f_size * f_size, output_channels])
        f_maps = tf.nn.relu(f_maps)
        cam = tf.matmul(f_maps, label_w, adjoint_b=True)
    ...
    def _build_graph(self):
        ...
        self.cam_att, self.viz_cam = self.generate_cam(self.pred_labels, f_maps1)

        self.input2 = tf.stop_gradient(get_resampled_images(
            self.high_res_images, self.cam_att, self.batch_size, self.src_size, self.dst_size, padding_size = 30, lamda = self.lamda))
    ...
    def train_all_epochs(self, train_params):
        ...
        try:
            ...
            dataframe.to_csv("./acc_results/B2_cam/" + self.which_split + ".csv", index = True, sep = ',')
    ...
    def test(self, batch_size):
        ...
        input_path = os.path.join("./visualize_cam/input", self.which_split)
        # saliency_path = "./visualize_conf_maps/saliency_save/B2 + 3TOA/split2/"
        saliency_path = "./visualize_attention/B2_cam_relu"
        ...
        for i in range(self.num_test // batch_size):
            ...
            #
            #         save_img(cam_att1[index], img_index, saliency_path, img_name = '.jpg', split = self.which_split, mode = "image")
                    # save_img(avg_att1[index], img_index, saliency_path, img_name='_avg.jpg', split=self.which_split, mode="image")
                    # save_img(avg_att1[index], img_index, saliency_path, img_name = '_avg_att1.jpg', mode = "image")
                    # save_img(test_low_images[index], img_index, saliency_path, img_name = '_input1.jpg', mode = "image")
                    # save_img(input2[index], img_index, saliency_path, img_name = '_input2.jpg', mode = "image")
