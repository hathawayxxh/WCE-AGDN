"""
This script replaces the DAC loss in the AGDN model with the mutual learning mechanism in the paper "Deep Mutual Learning".
Instead of using the DAC loss proposed in the paper, the two branches learn the logits predictions of each other.
"""
...
class DenseNet:
    ...
    def save_path(self):
        try:
            ...
            save_path = os.path.join(save_path, 'model.chkpt-1-1')
    ...
    def kl_loss_compute(self, pred1, pred2):
        """ KL loss to regulate the predictions of the two branches to learn from each other.
        """
        kl_loss = tf.reduce_mean(tf.reduce_sum(pred2 * tf.log(1e-8 + pred2 / (pred1 + 1e-8)), 1))
        return kl_loss
    ...
    def _build_graph(self):
        ...
        kl_loss1 = self.kl_loss_compute(self.pred1, self.pred2)
        kl_loss2 = self.kl_loss_compute(self.pred2, self.pred1)
        ...
        self.train_step1 = optimizer.minimize(
            self.cross_entropy1 + kl_loss1 + l2_loss1 * self.weight_decay, var_list = var_list1)
        self.train_step2 = optimizer.minimize(
            self.cross_entropy2 + kl_loss2 + l2_loss2 * self.weight_decay, var_list = var_list2)
    ...
    def train_all_epochs(self, train_params):
        ...
        self.test_high_image_batch, self.test_low_image_batch, self.test_label_batch = \
                                    get_image_label_batch(batch_size, self.which_split, shuffle=False, name='test3')
        ...
        try:
            ...
            dataframe.to_csv("./acc_results/3TLFA_DML_finetune/" + self.which_split + ".csv", index = True, sep = ',')
    ...
    def test(self, batch_size):
        ...
        saliency_path = "./visualize_attention/3TLFA_DML_finetune"
        ...
        for i in range(self.num_test // batch_size):
            ...
            for index in range(batch_size):
                ...
                if labels[index] != 0:
                    save_img(s_map1[index], img_index, saliency_path, img_name = '.jpg', split=self.which_split, mode = "image")
