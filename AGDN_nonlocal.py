"""
Replace the 3 DTOA modules with nonlocal modules.
"""
...
class DenseNet:
    ...
    def save_path(self):
        try:
            ...
            save_path = os.path.join(save_path, 'model.chkpt-70')
    ...
    def AAM(self, x, de=4, scope='AAM', trainable=True, reuse=False):
        with tf.variable_scope(scope, reuse=reuse):   
            channels = int(x.shape[-1])     
            f, _ = deform_con2v(x, num_outputs=channels // de, kernel_size=3, \
                stride=1, trainable=trainable,  name=scope+'f_conv', reuse=reuse)
            g, _ = deform_con2v(x, num_outputs=channels // de, kernel_size=3, \
    ...
    def nonlocal_block(self, x, de=4, scope='Nonlocal', trainable=True, reuse=False):
        with tf.variable_scope(scope, reuse=reuse): 
            ...
            ## Embedded Gaussian 
            f = conv(x, channels // de, kernel=1, stride=1, sn=True, scope='f_conv')
            g = conv(x, channels // de, kernel=1, stride=1, sn=True, scope='g_conv')

            # f = tf.nn.relu(f) #[bs, h, w, c_num]
            # g = tf.nn.relu(g) #[bs, h, w, c_num]

            # f = conv(x, channels // de, kernel=3, stride=1, pad = 1, sn=True, scope='f_conv')
            # g = conv(x, channels // de, kernel=3, stride=1, pad = 1, sn=True, scope='g_conv')
    ...
    def SONA(self, x, de=4, scope='SONA', trainable=True, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):  
            ...
            correlation1 = tf.matmul(tf.matmul(hw_flatten(f), I_hat), hw_flatten(f), False, True)

            # correlation1 = tf.matmul(hw_flatten(f), hw_flatten(f), transpose_b=True) # # [bs, N, N]
            correlation = tf.nn.softmax(correlation1, axis = -1)
    ...
    def network(self, _input):
        ...
        for block in range(self.total_blocks):
            ...
            if block == 0:
                cor1, output = self.nonlocal_block(output, de=4, scope='NLB1', trainable=True, reuse=False)
                # output = self.AAM(output, de = 4, scope = 'AAM1')
                # cor1, output = self.SONA(output, de=4, scope='SONA1', trainable=True, reuse=False)

            if block == 1:
                cor2, output = self.nonlocal_block(output, de=4, scope='NLB2', trainable=True, reuse=False)
                # output = self.AAM(output, de = 4, scope = 'AAM2')
                # cor2, output = self.SONA(output, de=4, scope='SONA2', trainable=True, reuse=False)
                fmaps_b2 = output

            if block == 2:
                cor3, output = self.nonlocal_block(output, de=4, scope='NLB3', trainable=True, reuse=False)
                # output = self.AAM(output, de = 4, scope = 'AAM3')
                # cor3, output = self.SONA(output, de=4, scope='SONA3', trainable=True, reuse=False)
                fmaps_b3 = output
        ...
        return f_maps, fmaps_b2, fmaps_b3, features, logits, spatial_pred, cor1, cor2, cor3
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope("net1") as scope:
            f_maps1, fmaps1_b2, fmaps1_b3, self.features1, logits1, spatial_pred1, \
                self.cor1, self.cor2, self.cor3 = self.network(self.images)
            # print ("correlation in nonlocal_block2:", self.cor2)
            self.pred1 = tf.nn.softmax(logits1)
            self.f_maps1 = tf.nn.relu(f_maps1)
            self.fmaps1_b3 = tf.nn.relu(fmaps1_b3)
        ...
        with tf.variable_scope("net2") as scope:
            f_maps2, fmaps2_b2, fmaps2_b3, self.features2, logits2, spatial_pred2, _, _, _ = self.network(self.input2)
    ...
    def train_all_epochs(self, train_params):
        ...
        self.test_high_image_batch, self.test_low_image_batch, self.test_label_batch = \
                                    get_image_label_batch(batch_size, shuffle=False, name='test2')
    ...
    def test(self, batch_size):
        ...
        # saliency_path = "./visualize_conf_maps/saliency_save/B2 + 3nonlocal/split4/"
        saliency_path = "./visualize_conf_maps/saliency_maps/"
        cor_maps_path = "./visualize_conf_maps/correlation_maps/"
        ...
        for i in range(self.num_test // batch_size):
            ...
            fetches = [self.input2, self.cross_entropy1, self.cross_entropy2, self.sum_loss, self.TE_loss, \
                        self.saliency1, self.saliency2, self.smaps1_b2, self.smaps1_b3, self.smaps2_b2, \
                        self.smaps2_b3, self.pred1, self.pred2, self.pred, self.cor2, self.fmaps1_b3]

            input2, loss1, loss2, loss3, TE_loss, s_map1, s_map2, smap1_b2, smap1_b3, smap2_b2, smap2_b3, \
                        pred1, pred2, pred, NL2_cor, f_b3 = self.sess.run(fetches, feed_dict=feed_dict)
            ...
            for index in range(batch_size):
                ...
                save_img(smap1_b2[index], img_index, saliency_path, img_name = '_net1_b2.jpg', mode = "image")
                save_img(smap1_b3[index], img_index, saliency_path, img_name = '_net1_b3.jpg', mode = "image")

                save_img(smap2_b2[index], img_index, saliency_path, img_name = '_net2_b2.jpg', mode = "image")
                save_img(smap2_b3[index], img_index, saliency_path, img_name = '_net2_b3.jpg', mode = "image")


            # img1_maps = NL2_cor[1,:,:] #[256, 256]
            # save_img(test_low_images[1], 0, cor_maps_path, img_name = '_image.jpg', mode = "image")
            # count = 0
            # for i in range(1,16):
            #     for j in range(1,16):
            #         count += 1
            #         position = i * j
            #         att = img1_maps[position, :] #[256]
            #         print (position, ":", att)
            #         att_map = np.tile(np.expand_dims(np.reshape(att, [16, 16]), axis = -1), [1,1,3])
            #         att_map = cv2.resize(att_map, (128, 128))
            #         att_map = (att_map - np.min(att_map))/(np.max(att_map) - np.min(att_map) + 1e-8)
            #         save_img(att_map, count, cor_maps_path, img_name = '.jpg', mode = "heatmap")

            #         image = test_low_images[1]
            #         image[(8*i-2):(8*i+2), (8*j-2):(8*j+2)] = [0,255,0]
            #         save_img(image, count, cor_maps_path, img_name = '_image.jpg', mode = "image")


            # feature = np.reshape(f_b3[1,:,:,:], [64, -1])
            # for i in range(1,64):
            #     print (i, np.sum(feature[i,:]), ":", feature[i,:])
            #     # for j in range(1,64):
            #     print ("Correlation matrix:", NL2_cor[1,i,:])
