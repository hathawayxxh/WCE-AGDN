...
then use attention guided deformation module to convert the orginal image with size = 320 to input2 with size = 128. 
The weights of two networks are independent.
...
class DenseNet:
    ...
    def TLFA(self, x, channels, de=2, scope='TLFA', trainable=True, reuse=False):
        """
    ...
    def network(self, _input):
        ...
        for block in range(self.total_blocks):
            ...
            if block == 0:
                output = self.TLFA(output, int(output.shape[-1]), de=2, scope='TLFA1', trainable=True, reuse=False)

            if block == 1:
                output = self.TLFA(output, int(output.shape[-1]), de=2, scope='TLFA2', trainable=True, reuse=False)
                fmaps_b2 = output

            if block == 2:
                output = self.TLFA(output, int(output.shape[-1]), de=2, scope='TLFA3', trainable=True, reuse=False)
                fmaps_b3 = output
    ...
    def _build_graph(self):
        ...
        # var_DTOA = [var for var in tf.trainable_variables() if var.name.split('/')[1] == 'DTOA1' 
        #             or var.name.split('/')[1] == 'DTOA2' or var.name.split('/')[1] == 'DTOA3' ]
        # print var_DTOA
